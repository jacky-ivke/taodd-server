...
public class DepositOrderAction {
    ...
    @ApiOperation("获取代理存款记录")

    @RequestMapping(value = "/agent/orders/deposit", produces = "application/json; charset=UTF-8", method = {RequestMethod.GET})
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "type", value = "类型", paramType = "query", dataType = "String", required = false),
            @ApiImplicitParam(name = "pageSize", value = "页大小", paramType = "query", dataType = "Integer", required = false),
            @ApiImplicitParam(name = "okStatus", value = "状态（0-处理中，1-成功，2-失败）", paramType = "query", dataType = "Integer", required = false),
    ...
    public JsonWrapper getDepositOrders(HttpServletResponse response, HttpServletRequest request, String type, Integer page, Integer pageSize, String startTime, String endTime, Integer okStatus) {
        ...
        PageData data = depositOrderManager.getOrders(account, type, okStatus, page, pageSize, startTime, endTime);
