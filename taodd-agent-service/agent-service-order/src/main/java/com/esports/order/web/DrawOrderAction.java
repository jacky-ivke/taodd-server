...
public class DrawOrderAction {
    ...
    @ApiOperation("获取代理提款记录")
    @RequestMapping(value = "/agent/orders/draw", produces = "application/json; charset=UTF-8", method = {RequestMethod.GET})
    @ApiImplicitParams({
            @ApiImplicitParam(name = "token", value = "认证令牌", paramType = "header", dataType = "String", required = true),
            @ApiImplicitParam(name = "type", value = "类型", paramType = "query", dataType = "int", required = false),
    ...
    public JsonWrapper getDrawOrders(HttpServletResponse response, HttpServletRequest request, Integer type, Integer page, Integer pageSize, String startTime, String endTime, Integer okStatus) {
        ...
        PageData data = drawOrderManager.getOrders(account, okStatus, type, page, pageSize, startTime, endTime);
