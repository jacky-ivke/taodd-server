package com.esports.external.handler.xint.leg;
...
import com.esports.external.handler.xint.ky.XintKyCode;
import com.esports.external.handler.xint.leg.dto.*;
...
@Slf4j
public class XintLegProxy extends AbstractTemplate {

    private final static String TYPE = "LEG";
    ...
    protected RespResultDTO buildResult(String errorCode) {
        ...
        respResult.setMessage(XintLegCode.getMessage(errorCode));
        return respResult;
    }
    ...
    public RespResultDTO balance(String loginName, String ip) {
        ...
        try {
            ...
            if (null == config || StringUtils.isEmpty(loginName)) {
                return this.buildResult(XintLegCode._PARAM_ERROR.getCode());
            }
            ...
            respResult = this.buildBalanceResult(respResult, XintLegCode._PARAM_ERROR.getCode());
            log.info("【获取余额】平台：XINTLEG,账号：{},结果：{}", loginName, respResult);
        } catch (Exception e) {
            respResult = this.buildResult(XintLegCode._EXCEPTION.getCode());
            log.error("【获取余额】平台：XINTLEG, 账号：{}, 异常信息：{}", loginName, e.getMessage());
        }
        return respResult;
    }
    ...
    public RespResultDTO deposit(String loginName, Integer lang, String orderNo, BigDecimal amount, Integer deviceType, String ip, Long timestamp) {
        ...
        try {
            ...
            log.info("【转账上分】平台：XINTLEG, 账号：{}, 结果：{}", loginName, respResult);
        } catch (Exception e) {
            log.error("【转账上分】平台：XINTLEG, 账号：{}, 异常信息：{}", loginName, e.getMessage());
        } finally {
    ...
    public RespResultDTO draw(String loginName, Integer lang, String orderNo, BigDecimal amount, Integer deviceType, String ip, Long timestamp) {
        ...
        try {
            ...
            dto.setMoney(formatAmount(amount));
            respResult = this.handler(dto, config, timestamp, null);
            log.info("【转账下分】平台：XINTLEG, 账号：{}, 结果：{}", loginName, respResult);
        } catch (Exception e) {
            log.error("【转账下分】平台：XINTLEG, 账号：{}, 异常信息：{}", loginName, e.getMessage());
        } finally {
            respResult = this.checkTransfer(respResult, loginName, lang, orderNo, amount, timestamp);
        }
        return respResult;
    ...
    public RespResultDTO searchOrder(String loginName, String orderNo, Integer lang, Long timestamp) {
        ...
        try {
            ...
            log.info("【转账查询】平台：XINTLEG, 账号：{}, 结果：{}", loginName, respResult);
            String status = respResult.getData().toString();
            String errorCode = respResult.getCode();
            if (!respResult.getSuccess() || !XintLegCode.OrderStatus._SUCCESS.getCode().equals(status)) {
            ...
            respResult = this.buildResult(XintLegCode._EXCEPTION.getCode());
            log.error("【转账查询】平台：XINTLEG, 账号：{}, 异常信息：{}", loginName, e.getMessage());
        }
        return respResult;
    }
    ...
    public RespResultDTO playGame(String loginName, Integer deviceType, String memberIp, Integer lang, String gameCode, String gameType, BigDecimal amount, String transferNo, Long timestamp) {
        ...
        try {
            ...
            log.info("【进入游戏】平台：XINTLEG, 账号：{}, 结果：{}", loginName, respResult);
        } catch (Exception e) {
            respResult = this.buildResult(XintLegCode._EXCEPTION.getCode());
            log.error("【进入游戏】平台：XINTLEG,账号：{} , 异常信息：{}", loginName, e.getMessage());
        }
        return respResult;
    }
}
