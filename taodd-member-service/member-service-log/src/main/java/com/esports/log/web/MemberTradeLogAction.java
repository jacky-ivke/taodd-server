...
@Api(tags = {"【会员管理系统-日志记录相关接口】"})
@RestController
public class MemberTradeLogAction {

    @Autowired
    private MemberTradeManager memberTradeManager;

    @ApiOperation("获取会员交易日志记录")

    @RequestMapping(value = "trade", produces = "application/json; charset=UTF-8", method = {RequestMethod.GET})
    ...
    public JsonWrapper getOrders(HttpServletResponse response, HttpServletRequest request, Integer okstatus, String type, Integer page, Integer pageSize, String startTime, String endTime) {
        ...
        PageData data = memberTradeManager.getOrders(account, okstatus, type, page, pageSize, startTime, endTime);
